"""Repeatedly run the LCF orchestrator pipeline at a configurable interval.

Usage:
    python news_stock_tracker/run_loop.py --interval 15 --stocks 11 --mode adaptive
    python news_stock_tracker/run_loop.py --interval 30 --auto-trade

Runs the full orchestrator pipeline (run_orchestrator_pipeline.py) in a loop
with sleep between iterations. Monitors portfolio holdings and discovers new
...
PIPELINE_SCRIPT = os.path.join(_SCRIPT_DIR, "run_orchestrator_pipeline.py")


def main():
    import argparse
    parser = argparse.ArgumentParser(description="LCF Pipeline Loop Runner")
    ...
    parser.add_argument("--stocks", type=int, default=11,
                        help="Number of stocks per run (default: 11)")
    ...
    parser.add_argument("--whatsapp", type=str, default="",
                        help="WhatsApp number for alerts")
    ...
    print(f"\n{'#'*60}")
    print(f"#  LCF Orchestrator LOOP Runner")
    ...
    while True:
        ...
        start = datetime.now()
        print(f"\n{'='*60}")
        print(f"  LOOP ITERATION #{run_count} — {start.strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"{'='*60}")
        ...
        try:
            ...
            print(f"\n[LOOP] Run #{run_count} finished in {elapsed:.0f}s — {status}")
        except subprocess.TimeoutExpired:
            print(f"\n[LOOP] Run #{run_count} TIMED OUT after 600s")
        except KeyboardInterrupt:
            print(f"\n[LOOP] Interrupted by user. Exiting.")
            break
        except Exception as e:
            print(f"\n[LOOP] Run #{run_count} ERROR: {e}")
        ...
        print(f"[LOOP] Next run at {next_run_str} (sleeping {args.interval} min)...")
        print(f"{'-'*60}")
